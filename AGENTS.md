...
- Для операций с Postman сначала используй доступный Postman MCP. Ручной интерфейс применяй там, где MCP не даёт нужной операции, включая MCP Generator.
- Не делай существующее личное/командное workspace публичным. Для пилота используй отдельное workspace.
- До официальной передачи публичные Postman-артефакты маркируй `experimental` и `unofficial`.
...
1. публикация или изменение публичного Postman workspace/API/collection;
2. любой live-запрос, который меняет данные, включая тестовую школу;
3. destructive или sensitive live-вызов;
4. передача репозитория или смена владельца;
5. deployment в staging/production, изменения DNS, secrets или инфраструктуры;
6. снятие маркировки `experimental/unofficial`;
7. публикация в Postman MCP Catalog или других публичных каталогах.
8. публичное включение `LoginPupil` и `LoginTeacher`: покажи recorded product/security approval и отдельно получи подтверждение Руслана.
...
- Всегда явно разделяй: подготовлено локально, опубликовано в Postman, развёрнуто на staging, развёрнуто в production.
