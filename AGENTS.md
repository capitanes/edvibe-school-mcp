...
- Для HTTP-клиента поддерживай опциональный `X-Edvibe-Client-Id: <UUID v4>`; старые конфиги без него должны продолжать работать.
...
## Безопасная HTTP-телеметрия

- Централизованная телеметрия относится только к Streamable HTTP. Не добавляй её в STDIO и не заявляй доступ к исходному пользовательскому вопросу.
- Разрешены только события `service_started`, `mcp_initialize_completed`, `mcp_request_rejected`, `tool_call_completed`, `service_stopped`, `telemetry_storage_degraded` и поля строгой схемы из `src/telemetry/schema.js`.
- Запрещено передавать логгеру headers, IP/User-Agent, URL/query, arguments/results, request/response body, API-ключ, raw-домен, UUID, токены, пароли, JSON-RPC body/id, `error.message`, stack и произвольные поля.
- Школу и установку псевдонимизируй только HMAC-SHA256. Установка tenant-scoped; один UUID нельзя использовать для связи школ. При ротации HMAC обязательно увеличивай `TELEMETRY_IDENTITY_EPOCH`.
- HMAC-секрет и пароль дашборда храни только через systemd `LoadCredential`; не добавляй секретные env fallback.
- HTTP-поток должен только валидировать событие и ставить его в bounded-очередь; SQLite, journald и аналитические запросы выполняй в отдельном worker thread. Деградация телеметрии не должна менять MCP-ответ. Details — максимум 90 дней; агрегаты без request ID и identity — максимум 365 дней; backup очищай по тем же границам.
- `/admin/analytics` — только GET, Basic Auth с пользователем `analytics`, HTTPS, `no-store`, строгий CSP, без внешних скриптов и iframe.
- `/healthz` не входит в продуктовую статистику и не должен менять ответ.
- Tenant allowlist и фактическое отключение `LoginPupil` / `LoginTeacher` — отдельные security-задачи, не считай их решёнными телеметрией.
- По умолчанию `TELEMETRY_ENABLED=false` и `ANALYTICS_DASHBOARD_ENABLED=false`. Не включай их и не создавай production credentials без отдельного подтверждения Руслана.
...
curl -s -X POST https://edvibe.sungurov.com/mcp \
  ...
  -H "X-Edvibe-Client-Id: <UUID_V4_PER_INSTALLATION>" \
...
- `*.sqlite`, WAL/SHM и локальные backup — в `.gitignore`; telemetry database и credentials никогда не синхронизируются через Git.
...
### Что предлагать Руслану после каждого изменения в репозитории

После любых правок в файлах репозитория `/home/ruslan/Documents/Projects/edvibe-school-mcp/` агент **обязан** предложить Руслану выбор из четырёх вариантов и пояснить, где в итоге окажутся изменения при каждом варианте:

#### Вариант 1 — оставить изменения без коммита

- Изменения остаются только в рабочих файлах локального репозитория.
- В локальной git-истории, на GitHub и на сервере остаётся старая версия.
- Когда использовать: когда Руслан хочет сначала просмотреть или дополнить изменения.

#### Вариант 2 — только закоммитить (локально)
...
#### Вариант 3 — закоммитить и запушить на GitHub
...
#### Вариант 4 — закоммитить, запушить и задеплоить на сервер

- Изменения сохраняются в локальной git-истории (`git commit`).
- Изменения уходят на GitHub (`git push origin main`).
- Сервер подтягивает изменения, перезапускает сервис, проверяет health (`./scripts/deploy.sh`).
- Когда использовать: когда правка влияет на runtime (`src/`, `manifest/`, `package.json`) и нужно, чтобы `https://edvibe.sungurov.com/mcp` работал с новым кодом.
- Деплой ручной — `deploy.sh` запросит пароль SSH. Руслан вводит его сам в своём терминале. Агент не может запустить `deploy.sh` сам, потому что не может ввести пароль SSH интерактивно.
- **Команду для деплоя агент даёт ТОЛЬКО после того, как Руслан выбрал вариант 4 и отдельно подтвердил деплой** — не в момент предложения вариантов, а следующим сообщением после выбора. Команда:

  ```bash
  cd /home/ruslan/Documents/Projects/edvibe-school-mcp && ./scripts/deploy.sh
  ```

Руслан пока новичок в GitHub, поэтому при каждом выборе агент должен кратко пояснять, что произойдёт, и не использовать git-жаргон без объяснения.

Автоматический деплой через GitHub Actions **не используется**: на бесплатном плане GitHub Actions не запускаются для приватных репозиториев (`startup_failure` с zero jobs). Файлы `.github/workflows/deploy.yml` и `.github/workflows/test-minimal.yml` оставлены в репозитории, но они не выполняются. Не предлагай настраивать автодеплой — Руслан деплоит вручную через `./scripts/deploy.sh`.
