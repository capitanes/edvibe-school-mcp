...
- **Команду для деплоя агент даёт ТОЛЬКО после того, как Руслан выбрал вариант 3** — не в момент предложения вариантов, а следующим сообщением после выбора. Команда:
  ```bash
  cd /home/ruslan/Documents/Projects/edvibe-school-mcp && ./scripts/deploy.sh
  ```

**Вариант 4 — ничего не делать**
- Изменения остаются в рабочих файлах, не коммитятся.
- На GitHub — старая версия.
- На сервере — старая версия.
- Когда использовать: когда правки черновые, хочешь проверить ещё раз перед сохранением. Внимание: при следующем `git pull` или `git checkout` незакоммиченные изменения могут потеряться.
