...
tags:
  ...
  - postman
created: 2026-08-22
updated: 2026-08-22 (добавлены описания GroupClassPupilsAdd и PupilsDelete в operation-meta.cjs)
---
...
- ~~работа в интерфейсе Postman MCP Generator~~ — не требуется, сервер написан вручную (см. PLAN.md → Этап 3);
...
Демо-данные, созданные в ходе проб и оставленные в школе: учителя `3577063`–`3577067`, ученики `3577069`–`3577073`. Все записи созданы с `isActive=false` (часть позже активирована назначением уроков) и синтетическими email на зарезервированном домене `example.com` (RFC 2606). Записи остаются в школе как демо-набор; удаление не планируется.
...
| `ScheduleCreateLesson` | Авто-активация срабатывает только для индивидуальных уроков. | В схеме нет различия между индивидуальным и групповым уроком. | При создании индивидуального урока с `pupilId` ответ содержит `isActivatedPupil: true`. При создании группового урока без `pupilId` ответ содержит `isActivatedPupil: false` — авто-активация не срабатывает. | Подтвердить, что авто-активация привязана к наличию `pupilId` в запросе, и уточнить это в описании операции. |

Демо-данные, созданные в ходе этих проб и оставленные в школе: индивидуальные классы `2354995`–`2355000` (с пропуском `2354998`, который upstream зарезервировал/не вернул), уроки `29459741`–`29459745`. Все уроки запланированы на `2026-08-23` 10:00–14:00 UTC, по 60 минут. Записи остаются в школе как демо-набор; удаление не планируется.
...
Демо-данные, созданные в ходе этих проб и оставленные в школе: групповые классы `2355017` («English A2 (demo)», учитель Zhanna, ученики Anna/Viktor/Dmitry) и `2355018` («English B1 (demo)», учитель Kirill, ученики Boris/Galina); групповые уроки `29459865` и `29459866` (English A2, 2026-08-23/24 16:00 UTC), `29459871` и `29459872` (English B1, 2026-08-23/24 17:00 UTC), все по 60 минут. ~~Записи остаются в школе как демо-набор; удаление не планируется.~~ **Удалено 22 августа 2026 года по явному запросу Руслана** для освобождения лимита активированных учеников: ученики `3577069`–`3577073` отвязаны от групповых классов `2355017`/`2355018` и удалены, индивидуальные классы `2354995`–`2354997`, `2354999`, `2355000` удалены. Пустые групповые классы `2355017`/`2355018` оставлены. Это расходится с прежним решением «не удалять» и фиксируется как обновление.

### Дополнительные live-наблюдения (лимит активированных учеников + GroupClassPupilsAdd)

Зафиксировано 22 августа 2026 года на `edvibe.com` через `GroupClassPupilsAdd` и `PupilsDelete`.

| operationId | Наблюдение | Что говорит upstream | Что происходит фактически | Вопрос product/engineering |
|---|---|---|---|---|
| `GroupClassPupilsAdd` | Добавление ученика в групповой класс активирует его и расходует тарифный лимит активированных учеников школы. | В OpenAPI нет упоминания активации или лимита. | На тарифе Pro Trial 1 мес. лимит — 10 активированных учеников. `GroupClassPupilsAdd` для 20 новых учеников (`isActive=false`) падает с HTTP 400 `isSuccess=false`, `errorMessage="The number of activated students has been exceeded"`. После отвязки/удаления 5 старых демо-учеников добавление 10 новых в 5 классов прошло успешно, остальные 10 остались неактивными. | Подтвердить, что `GroupClassPupilsAdd` активирует ученика и расходует тарифный лимит. Если да, отразить это в description операции и в onboarding MCP. Уточнить, где читать текущий лимит и остаток (UI тарифа: «Онлайн-уроков 5/10»). |
| `PupilsDelete` | Удаление ученика блокируется, пока у него есть классы. | В OpenAPI нет упоминания блокировки. | HTTP 400 `isSuccess=false`, `errorMessage="Pupil have classes"`. Требуется сначала `GroupClassPupilsDelete` для каждого группового класса и `IndividualClassesDelete` для каждого индивидуального класса. | Подтвердить документированное поведение и добавить в description операции. |

Демо-данные, созданные 22 августа 2026 года по запросу Руслана и оставленные в школе: тариф `7582` (длительность `8804`, 60 мин); 20 учеников `3577117`–`3577136` (имена — президенты стран, кроме США, `isActive=false`, email на `example.com`), из них 10 активированы через `GroupClassPupilsAdd`; 5 преподавателей `3577137`–`3577141` (5 последних президентов США); 5 групповых классов `2355028`–`2355032` (Mathematics/Physics/Chemistry/Biology/History, по одному преподавателю на класс, в каждом по 10 учеников); 10 групповых уроков `29459957`–`29459966` по 60 минут, расписание на 2 недели вперёд (`2026-08-24` … `2026-09-05`, разное время UTC). Лимит активированных учеников исчерпан (10/10). Записи остаются в школе как демо-набор; удаление не планируется.
...
## Postman: опциональная публикация (не блокирует запуск)

- Postman MCP Catalog даёт только discoverability: школы находят сервер поиском в Cursor/Codex вместо ручного ввода URL. Onboarding через help center/инструкцию работает без Catalog.
- Postman Collection генерируется локально скриптом `scripts/generate-postman-collection.cjs` — Postman-аккаунт не требуется для пилота.
- Postman MCP Generator **не используется** — MCP-сервер написан вручную с security-слоем (SSRF, rate limit, BaseResponse normalization, credential context). См. PLAN.md → Этап 3.
- Публикация в Postman MCP Catalog — опциональный шаг после запуска production, если Edvibe решит, что Catalog нужен. Требует профиль издателя Edvibe, подтверждение домена и обращение на `api-network@postman.com`.
- Gate A (Public Postman) и Gate E (Publication) убраны из обязательных gates — см. PLAN.md → Approval gates → опциональные шаги.
...
- Личный HTTP-стенд Руслана (до официальной передачи Edvibe): `https://edvibe.sungurov.com/mcp`, Streamable HTTP transport на порту 9000, сервер `185.180.230.233`, путь на диске `/var/www/edvibe.sungurov.com/edvibe-school-mcp/`, systemd-сервис `edvibe-mcp.service`. Не официальный staging/production, не принадлежит Edvibe. Используется только для тестов пилота; не нарушает Gate D, потому что это не staging/production из плана.
...
- опциональный `X-Edvibe-Client-Id: <UUID v4>` — постоянный идентификатор конкретной установки клиента для псевдонимной аналитики. Старые конфиги без него продолжают работать.
...
#### Авторизация доступа к HTTP-endpoint (вариант A)

Для личного стенда `edvibe.sungurov.com` выбран вариант A — без отдельного access-токена к MCP-серверу. Единственный ключ — это `EDVIBE_API_KEY` школы:

- клиент шлёт `Authorization: Bearer <EDVIBE_API_KEY>` в каждом запросе к `https://edvibe.sungurov.com/mcp`;
- MCP-сервер снимает `Bearer ` префикс и использует тот же ключ как для upstream-вызова к Edvibe, так и (косвенно) как для авторизации доступа к себе: невалидный ключ отбивается на первом же upstream-вызове с 401 от Edvibe;
- отдельного `MCP_ACCESS_TOKEN` нет — школа вводит только один ключ в конфиге MCP-клиента;
- защита от спама с мусорными ключами: пока не реализована (невалидные ключи создают 401-нагрузку на upstream). Если станет проблемой — добавить rate limit по IP или опциональный access-токен позже.

Это решение действует только для личного стенда `edvibe.sungurov.com`. Для официального staging/production на `mcp-staging.edvibe.com` / `mcp.edvibe.com` модель авторизации пересматривается при передаче Edvibe.
...
### Безопасная телеметрия личного HTTP-стенда — 2026-09-24

Локально реализована опциональная телеметрия только для Streamable HTTP. На личном стенде сбор и дашборд ещё не включены: первый деплой должен выполняться с `TELEMETRY_ENABLED=false` и `ANALYTICS_DASHBOARD_ENABLED=false`, дальнейшее включение требует отдельного подтверждения.

- Исходные вопросы пользователя и локальный STDIO серверу недоступны и не учитываются.
- Школа и установка записываются только как tenant-scoped HMAC-SHA256 псевдонимы `t_…` / `i_…`; raw-домен и UUID не сохраняются.
- Логгер принимает только шесть типов событий и закрытый список безопасных полей. Headers, IP/User-Agent, URL/query, arguments/results, bodies, API-ключи, токены, пароли, JSON-RPC body/id, произвольные сообщения ошибок и stack запрещены схемой.
- HMAC-секрет и пароль дашборда читаются только через systemd `LoadCredential`; при ротации HMAC увеличивается `TELEMETRY_IDENTITY_EPOCH`.
- HTTP-поток только валидирует событие и ставит его в ограниченную асинхронную очередь; SQLite, journald и аналитические запросы выполняются в отдельном worker thread. Ошибка хранения не меняет MCP-ответ. Для деталей действует максимум 90 дней, для агрегатов без request ID и псевдонимов — максимум 365 дней; те же границы ежедневно применяются к сохранённым backup.
- `X-Forwarded-Proto: https` учитывается только от loopback/private proxy, поэтому порт 9000 должен быть закрыт от публичного доступа. Проверенный публичный IPv4 upstream закрепляется в HTTPS-соединении, чтобы повторный DNS lookup не обходил SSRF-проверку.
- Закрытый русскоязычный дашборд `/admin/analytics` использует отдельный Basic Auth, только GET, `Cache-Control: no-store`, CSP и запрет iframe.
- Tenant allowlist и фактическое отключение `LoginPupil` / `LoginTeacher` остаются отдельными security-задачами и не подменяются телеметрией.
...
- **Per-key rate limiter**: 10 запросов в секунду и максимум 4 одновременных запроса **на каждый API-ключ**. Школы не блокируют друг друга; registry индексируется HMAC API-ключа, а не raw значением.
- **HTTP keep-alive**: переиспользование TCP+TLS-соединений через `https.Agent({ keepAlive: true })`, до 100 сокетов. Без keep-alive каждый запрос создавал новый handshake (~100–300ms overhead).
- **DNS-кэш**: `resolveHostname()` кэширует DNS resolution 60 секунд, удаляет истёкшие записи и ограничен 1000 hostname.
- Registry rate limiter хранит только HMAC API-ключа с процессным случайным ключом, имеет idle TTL 10 минут и предел 10 000 записей; raw API-ключ не используется как ключ Map.
