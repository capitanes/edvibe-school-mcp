...
- **Per-key rate limiter**: 10 запросов в секунду и максимум 4 одновременных запроса **на каждый API-ключ**. Школы не блокируют друг друга. Раньше лимитер был глобальным (один на процесс) — это вызывало зависание при 10+ одновременных школах. Исправлено: `getLimiter(apiKey)` в `src/upstream.js` создаёт отдельный `RateLimiter` для каждого ключа.
...
- **DNS-кэш**: `resolveHostname()` кэширует DNS resolution 60 секунд, предотвращая избыточные lookup-ы при каждом запросе.
